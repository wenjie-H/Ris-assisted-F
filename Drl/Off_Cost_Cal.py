...
def calculate_off_delay(nodes:list, BSs:list, off_strategies:list):
    ...
    for node in node_list:
        if node.offload_decision == 1:
            ...
            node.offload_decision = target_BS
...
def calculate_delay_state(state, BSs:list, off_strategies: list):
...
def calculate_linkquality(nodes:list, BSs:list):
    ...
    links = Network(node_list, BS_list)
    print(links.link_d2i)
    return [[int(x) for x in row] for row in links.link_d2i]
...
if __name__ == '__main__':
    ...
    nodes_BSs = generate_network(5, 9, 800)
    # print(nodes_BSs)
    nodes = nodes_BSs[0]
    BSs = nodes_BSs[1]
    # print(nodes, BSs)
    ...
    d = calculate_off_delay(nodes, BSs, np.full(45, 1))
    print(d)